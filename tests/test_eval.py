...
from evals.run_eval import REC_DIR, run
...
@pytest.mark.skipif(
    not (REC_DIR / "v4.json").exists(),
    reason="the v4 recording is pending a live record pass (needs a key)",
)
def test_recorded_gate_is_green():
    ...
    assert run(level="v4", mode="recorded") == 0
