...
again, until it answers or a control stops it.

Two controls keep it from running forever: a hard step ceiling, and a loop
detector that stops the same action repeating. It reuses the shared contracts
(`LLMClient`, `ToolRegistry`, `Caps`, `LoopDetector`), so it is real and not a
toy; nothing is hidden behind a framework.
"""
