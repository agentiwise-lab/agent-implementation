...
hard step ceiling, detection of a loop repeating the same action, and a
goal-met signal so the loop ends the moment the work is done.
...
class LoopDetector:
    ...
    def _key(call: ToolCall) -> str:
        # Hash the action, not just the tool name: same tool, different args is
