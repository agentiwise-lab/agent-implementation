"""The agent as a real LangGraph state machine.

The raw loop teaches the mechanism; this is the same agent built on LangGraph, the
runtime the module uses once state, checkpoints, and durability matter. It is a
real StateGraph: an `agent` node calls the model, a `tools` node runs the
requested tool, and a conditional edge loops between them until the model returns
a final answer or the recursion limit stops it. Persistence is a real SqliteSaver
checkpointer, so a thread's state survives the process and can resume.

The model still comes through the same `LLMClient` contract, so OpenRouter, the
fake, and the recorded client all drive the graph unchanged.
"""
